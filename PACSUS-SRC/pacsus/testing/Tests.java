...
class Tests
{
	...
	Regular_visitor_permit perm4 = new Regular_visitor_permit("Tomas", new Vehicle_info("76841"), "Andre",
		new Date(1), new Date(5));
    ...
    /**
     * This test if populateList() method returns a String array of permits in
     * Permit_list
     */
    @Test
    ...
    /**
     * This test check if warnings() successfully adds a warning when type==1 and
     * also checks if when warnings are 3 doesn't go over it and also checks if it's
     * possible to delete a warning if type==-1
     */
    @Test
    void warningsTest()
    {
    ...
    /**
     * This test checks if when unsuspendedPermit() is called, it successfully sets
     * the warnings back to 0. If warnings are less than 3, it checks if it didn't
     * set back to 0
     * 
     */
    @Test
    ...
    /**
     * This test makes sure that we can successfully update the permit of a permit
     * holder.
     * 
     */
    @Test
	...
	assertEquals(perm2, list.getPermit("John"));
    }

    /**
     * This test check if deltePermit() successfully deletes a permit from
     * Permit_list.
     * 
     */
    @Test
    ...
    /**
     * This test check if getPermitsByType() returns a ArrayList of a certain type
     * of permits.
     */
    @Test
	...
	arrayList.add(perm1);
	assertEquals(arrayList, list.getPermitsByType("Permanent_visitor_permit"));

	// Check if type Day_visitor_permit type works
	arrayList.clear();
	arrayList.add(perm2);
	assertEquals(arrayList, list.getPermitsByType("Day_visitor_permit"));

	// Check if type University_member_permit type works
	arrayList.clear();
	arrayList.add(perm3);
	assertEquals(arrayList, list.getPermitsByType("University_member_permit"));
    ...
    /**
     * This test checks if setPermitsHasEntered resets the attribute enteredToday of
     * all permits to false
     */
    @Test
